...
use crate::session_data::{SessionEvent, GridCell};
...
pub async fn analyze_jsonl_data(input_path: &Path, verbose: bool) -> Result<()> {
    ...
    println!("{:<6} {:<8} {:<12} {:<20} {:<20} {:<10}", "Event", "Time(ms)", "Type", "Incremental Cursor", "Batched Cursor", "Diff?");
    println!("{}", "-".repeat(90));
    ...
    for line in lines {
        ...
        match event {
            SessionEvent::RawPtyOutput { timestamp_begin, data, .. } => {
                ...
                if cursor_diff || verbose {
                    println!("{:<6} {:<8} {:<12} ({:>2},{:>3})          ({:>2},{:>3})          {:<10}", 
                        event_count,
                        timestamp_begin,
                        "RawOutput",
                        incremental_cursor.0, incremental_cursor.1,
                        batched_cursor.0, batched_cursor.1,
                ...
                if cursor_diff {
                    cursor_differences.push((timestamp_begin, incremental_cursor, batched_cursor, data.clone()));
                }
                
                if verbose && data.len() > 0 {
            ...
            SessionEvent::GridUpdate { timestamp_begin, cursor, .. } => {
                if verbose {
                    println!("{:<6} {:<8} {:<12} Grid cursor: ({:>2},{:>3})", 
    ...
    if !cursor_differences.is_empty() {
        ...
        for (i, (timestamp, inc_cursor, batch_cursor, data)) in cursor_differences.iter().enumerate().take(10) {
            println!("#{} at {}ms: Incremental({},{}) vs Batched({},{}):", 
                i+1, timestamp, inc_cursor.0, inc_cursor.1, batch_cursor.0, batch_cursor.1);
        ...
        if cursor_differences.len() > 10 {
            println!("   ... and {} more differences", cursor_differences.len() - 10);
        }
    }
    ...
    println!("Batched processing:     ({}, {})", final_batch.0, final_batch.1);
...
async fn analyze_timing_patterns(cursor_differences: &[(u32, (u16, u16), (u16, u16), Vec<u8>)]) -> Result<()> {
    ...
    if !timing_gaps.is_empty() {
        ...
        println!("  Average: {}ms, Min: {}ms, Max: {}ms", avg_gap, min_gap, max_gap);
    }
    ...
    for (timestamp, inc_cursor, batch_cursor, data) in cursor_differences {
        let data_str = String::from_utf8_lossy(data);
        
        // Look for common VT100 patterns that cause cursor issues
        if data_str.contains("\x1b[2K") && data_str.contains("\x1b[1A") {
            *sequence_patterns.entry("screen_clear_sequence").or_insert(0) += 1;
        }
    ...
    if !timing_gaps.is_empty() {
        ...
        println!("   - Suggested max timeout: {}ms (half of smallest gap)", suggested_timeout);
...
async fn analyze_cursor_return_sequences(
    ...
    for (event_num, timestamp, data) in all_events {
        let data_str = String::from_utf8_lossy(data);
    ...
    println!("Found {} status message events at positions: {:?}", 
        status_message_events.len(), status_message_events);
    
    // Analyze the 3-5 events AFTER each status message to see cursor movement patterns
    for status_event_num in status_message_events.iter().take(3) { // Analyze first 3 for brevity
        println!("\n📋 Events after status message at event #{}:", status_event_num);
        
        // Look at next 5 events
        for i in 1..=5 {
            let next_event_num = status_event_num + i;
            if let Some((_, timestamp, data)) = all_events.iter().find(|(num, _, _)| *num == next_event_num) {
                let data_str = String::from_utf8_lossy(data);
                ...
                let preview: String = data_str.chars()
...
fn analyze_input_stream(data: &[u8], incremental_cursor: (u16, u16), batched_cursor: (u16, u16)) {
    ...
    if data_str.contains("> ") && (data_str.contains("t") || data_str.contains("e") || data_str.contains("s")) {
        ...
        if let Some(input_start) = data_str.find("> ") {
            ...
            if let Some(input_end) = input_line.find('\n').or(input_line.find("│")) {
                ...
                println!("      Input line: {}", input_content.chars().take(80).collect::<String>());
                ...
                for c in input_content.chars().skip(2) { // Skip "> "
                    if c == '\x1b' {
                        in_escape = true;
                        continue;
                    }
                    ...
                    if !c.is_control() {
                        ...
                        if typed_text.len() > 10 { // Reasonable limit for typed text
                            break;
                        }
                    }
                }
                ...
                println!("      Typed text: '{}' ({} chars)", typed_text, actual_chars);
                println!("      Expected cursor column: {} (after '> {}')", expected_cursor_col, typed_text);
                println!("      Incremental cursor: ({}, {}) - {} columns from expected", 
                    incremental_cursor.0, incremental_cursor.1,
                    (incremental_cursor.1 as i32 - expected_cursor_col as i32).abs());
                println!("      Batched cursor:     ({}, {}) - {} columns from expected", 
                    batched_cursor.0, batched_cursor.1,
                    (batched_cursor.1 as i32 - expected_cursor_col as i32).abs());
            }
        }
    }
