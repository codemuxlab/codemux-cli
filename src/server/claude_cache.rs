            ...
            for entry in entries.flatten() {
                if let Some(name) = entry.file_name().to_str() {
                    ...
                    if name.starts_with(target_segment) {
                        ...
                        let name_alpha_parts: Vec<&str> =
                            name_parts.into_iter().filter(|s| !s.is_empty()).collect();
...
impl ClaudeProjectsCache {
    ...
    async fn scan_directory(&self) -> Result<()> {
        ...
        while let Some(entry) = entries.next_entry().await? {
            ...
            if path.is_dir() {
                ...
                let encoded_project_name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");

                let actual_project_path = match decode_claude_project_path(encoded_project_name) {
                    Some(path) => {
                        debug!(
                            "Found Claude project: {} -> {:?}",
                            encoded_project_name, path
                        );
                        path
                    }
                    None => {
                ...
                while let Some(file_entry) = project_entries.next_entry().await? {
                    ...
                    if file_path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
                        if let Ok(session) = self
                            .parse_session_file(&file_path, &actual_project_path)
                            .await
                        {
                            sessions.insert(session.session_id.clone(), session.clone());
                            count += 1;
                            debug!(
                                "Found session: {} for project {:?}",
                                session.session_id, actual_project_path
                            );
    ...
    async fn parse_session_file(
        ...
        let agent = self
            .detect_agent_from_file(file_path)
            .await
        ...
        EventKind::Create(_) | EventKind::Modify(_) => {
            ...
            for path in event.paths {
                if path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
                    ...
                    let project_path =
                        if !encoded_dir.is_empty() && path.parent() != Some(&base_path) {
                            decode_claude_project_path(encoded_dir).unwrap_or_else(|| {
                                debug!(
                                    "Failed to decode project path: {}, using parent directory",
                                    encoded_dir
                                );
