	...
	subscribeWithSelector((set, get) => ({
		...
		handleGridUpdate: (message: {
			...
			set((state) => {
				...
				if (
					message.size &&
					(message.size.rows !== state.size.rows ||
						message.size.cols !== state.size.cols)
				) {
				...
				if (
					message.cursor &&
					(message.cursor.row !== state.cursor.row ||
						message.cursor.col !== state.cursor.col)
				) {
					...
					if (cursorVisible) {
						cellsToUpdate.set(newCursorKey, {
							...newCursorCell,
							has_cursor: true,
						});
					}
				...
				if (
					typeof message.cursor_visible === "boolean" &&
					message.cursor_visible !== state.cursor_visible
				) {
		...
		resolveColor: (color, isBackground = false) => {
			...
			if (typeof color === "object") {
				...
				if ("Palette" in color && typeof color.Palette === "number") {
					...
					if (index < 16) {
						return (
							state.theme.colors[index] ||
							(isBackground ? state.theme.background : state.theme.foreground)
						);
					} else if (index < 232) {
