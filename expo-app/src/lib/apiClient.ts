...
const getBaseUrl = (): string => {
  // Use port 18765 for debug builds, 8765 for release builds
  // Check if we're running on the Expo dev server (port 8081)
  if (__DEV__) {
    // Development: Expo dev server on 8081, backend on 18765 (debug build)
  ...
  // Fallback for SSR or other environments (default to debug port in dev)
  return "http://localhost:18765";
};
