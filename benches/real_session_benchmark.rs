...
fn load_session_data() -> Vec<u8> {
    let jsonl_content =
        std::fs::read_to_string("test_session.jsonl").expect("Failed to read test_session.jsonl");
...
fn bench_vt100_real_session(test_data: &[u8]) {
    ...
    println!(
        "VT100 per iteration: {:?}",
        vt100_duration / iterations as u32
    );
    println!(
        "VT100 bytes/sec: {:.0}",
        (test_data.len() * iterations) as f64 / vt100_duration.as_secs_f64()
    );
}
...
fn bench_alacritty_real_session(test_data: &[u8]) {
    ...
    println!(
        ...
        alacritty_duration / iterations as u32
    );
    println!(
        "Alacritty bytes/sec: {:.0}",
        (test_data.len() * iterations) as f64 / alacritty_duration.as_secs_f64()
    );
}
...
fn bench_alacritty_chunked_real_session(test_data: &[u8]) {
    ...
    println!(
        ...
        alacritty_chunked_duration / iterations as u32
    );
    println!(
        "Alacritty chunked bytes/sec: {:.0}",
        (test_data.len() * iterations) as f64 / alacritty_chunked_duration.as_secs_f64()
    );
